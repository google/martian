...
func (s *Stream) sendData(id string, mt MessageType, i uint32, b []byte) {
	bl := uint32(len(b))

	f := newFrame(id, DataFrame, mt, 32+bl)
...
func (s *Stream) sendEnd(id string, mt MessageType) {
	f := newFrame(id, EndFrame, mt, 0)
...
func (bl *bodyLogger) Read(b []byte) (int, error) {
	...
	if n > 0 {
		bl.s.sendData(bl.id, bl.mt, atomic.AddUint32(&bl.index, 1)-1, b)
	}
	if err == io.EOF {
		bl.s.sendEnd(bl.id, bl.mt)
	}
